...
def admm(A, b, lamda, C=1, max_iter=1000, tol=1e-5, if_draw=True):
    ...
    r = []
    iterates.append(x)
    temp1 = np.linalg.inv(np.sum([A[i].T.dot(A[i]) for i in range(n)], axis=0) + C*np.eye((d2)))
    for _ in range(max_iter):
        x_old = x.copy()

        # 更新x
        x = temp1.dot(np.sum([A[i].T.dot(b[i]) for i in range(n)], axis=0) + C*y - v)
        # 更新y
        ...
        if np.linalg.norm(x - x_old, ord=2) < tol:
            break
        # print(np.linalg.norm(x - x_old, ord=2))
